...
opt = lapp[[
  -s,--save          (default "/home/animesh1/assignment1/checkpoints")      subdirectory to save logs
  ...
  --nDonkeys         (default 4)           number of data loading threads
...
if opt.network == '' then
  ...
  model_FCN:remove(24) -- Removing nn.Softmax
  model_FCN:remove(23) -- Removing nn.Linear(4096 -> 1000)
  model_FCN:remove(22) -- Removing nn.Dropout(0.500000)
  model_FCN:remove(19) -- nn.Dropout(0.500000)
  model_FCN:remove(16) -- nn.View(-1)
  local temp_module = nn.SpatialConvolution(4096, opt.classnum, 1, 1, 1, 1)
  weights_init(temp_module)
  model_FCN:add(temp_module)
  model_FCN:add(nn.ReArrange())
  model_FCN:add(nn.LogSoftMax())
  -- Removing first layer and updating it with 100x100 padding
