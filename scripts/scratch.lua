...
opt = lapp[[
  ...
  --saveFreq         (default 6)          save every saveFreq epochs
  ...
  --learningRateDecayFreq (default 10)   learning rate decay very number of epochs
]]
...
if opt.network == '' then
  ...
  model_FCN:add(nn.SpatialConvolution(512, 40, 1, 1, 1, 1))		
...
while epoch <= opt.maxEpoch do
  -- train/test
  train()
  if opt.learningRateDecay ~= 0 and epoch % opt.learningRateDecayFreq == 0 then		
